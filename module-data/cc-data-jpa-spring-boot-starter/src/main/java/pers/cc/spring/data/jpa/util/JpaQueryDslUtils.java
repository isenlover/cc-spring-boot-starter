...
public class JpaQueryDslUtils {
  ...
  public static BooleanExpression getNumberGoeExpression(NumberPath numberPath, Double value) {
    if (CommonUtils.isEmpty(value)) {
      return null;
    }
  ...
  public static BooleanExpression getNumberRangeExpression(NumberPath numberPath, String text) {
    ...
    Double[] split = Arrays.stream(text.split("-")).map(Double::valueOf).toArray(Double[]::new);
    return numberPath.between(split[0], split[1]);
  }

  public static BooleanExpression getNumberRangeExpression(StringPath stringPath, String text) {
    if (CommonUtils.isEmpty(text) || !text.contains("-")) {
      return null;
    }
    String[] split = text.split("-");
    return stringPath.between(split[0], split[1]);
  }
  ...
  public static BooleanExpression getFormatDateBetweenTodayExpression(DateTimePath datePath, Date from, Date to) {
    return Expressions.stringTemplate("DATE_FORMAT({0},'%Y-%m-%d')", datePath).between(DateUtils.getString(from), DateUtils.getString(to));
  }

  public static BooleanExpression getFormatDateEqualYesterdayExpression(DateTimePath datePath) {
    return Expressions.stringTemplate("DATE_FORMAT({0},'%Y-%m-%d')", datePath).eq(DateUtils.getAfterOrBeforeHourString(-24, DateUtils.dateFormat));
  }
