...
import pers.cc.spring.core.annotation.swagger.extension.annotation.ApiGroupProperties;
import pers.cc.spring.core.annotation.swagger.extension.annotation.ApiGroupProperty;
import pers.cc.spring.core.annotation.swagger.extension.util.ReflectUtils;
...
/**
 * 对于表单参数或者get请求参数转对象(@RequestParam)关于组的构建
 */
@Component
@Primary
@ConditionalOnBean(annotation = EnableSwagger2.class)
public class ModelAttributeParameterGroupExpander extends ModelAttributeParameterExpander {
  ...
  @Autowired
  public ModelAttributeParameterGroupExpander(
  ...
  public List<Parameter> expand(ExpansionContext context) {
    Class<?> currentGroupClazz = null;
    for (ResolvedMethodParameter methodParameter : context.getOperationContext().getParameters()) {
      /*找到当前参数类型对应methodParameter*/
      if (methodParameter.getParameterType().equals(context.getParamType())) {
        Optional<ApiGroup> annotation = methodParameter.findAnnotation(ApiGroup.class);
        if (annotation.isPresent()) {
          //设置当前的ApiGroup
          currentGroupClazz = annotation.get().value();
          break;
        }
      }
    }
    /*如果当前方法参数没有配置ApiGroup 则走之前的逻辑*/
    if (currentGroupClazz == null) {
      return super.expand(context);
    }

    List<Parameter> parameters = newArrayList();
    Set<PropertyDescriptor> propertyDescriptors = Sets.newHashSet();
    Class<?> paramTypeClass = context.getParamType().getErasedType();
          ...
          if (apiGroupProperty != null) {
            for (Class<?> groupClazz : apiGroupProperty.value()) {
              if (groupClazz.equals(currentGroupClazz)) {
                ...
                if (CommonUtils.isNotEmpty(apiGroupProperty.description())) {
    ...
    if (!apiGroupPropertyMap.isEmpty()) {
      parameters.forEach(parameter -> {
        ApiGroupProperty apiGroupProperty = apiGroupPropertyMap.get(parameter.getName());
        if (apiGroupProperty != null) {
          ReflectUtils.setPrivateField(parameter, "description", apiGroupProperty.description());
        }
      });
    }
  ...
  private List<Parameter> expand(ExpansionContext context, Set<PropertyDescriptor> propertyDescriptors) {
    List<Parameter> parameters = newArrayList();
