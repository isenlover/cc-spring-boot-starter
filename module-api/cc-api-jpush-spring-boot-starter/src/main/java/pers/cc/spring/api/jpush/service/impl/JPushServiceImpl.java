...
public class JPushServiceImpl implements JPushService {
  ...
  public static PushPayload buildPushObject_android_and_ios(String content, boolean volume, List<String> jPushIds) {
...
//    extras.put("test", "https://community.jiguang.cn/push");
    ...
    return PushPayload.newBuilder()
        ...
        .setAudience(Audience.newBuilder()
            .addAudienceTarget(AudienceTarget.alias(jPushIds.stream().filter(Objects::nonNull).collect(Collectors.toList())))
            .build()
        )
...
//                .addExtras(extras).build())
  ...
  // TODO 2022/6/27: 接口需要整理了
  @Override
  public void pushMessage(String message, List<String> jPushIds) {
    ClientConfig clientConfig = ClientConfig.getInstance();
    clientConfig.setApnsProduction(coreProperties.isJPushProduction());
    ...
    final PushPayload payload = buildPushObject_android_and_ios(message, false, jPushIds);
    try {
      PushResult result = jpushClient.sendPush(payload);
      log.info("JPush result - " + result + "，推送成功，消息：" + message);
...
//        jpushClient.getPushClient().setHttpClient(httpClient);
    final PushPayload payload = buildPushObject_android_and_ios(message, volume, Lists.newArrayList("cc", "footballApp"));
