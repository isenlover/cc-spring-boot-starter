...
public class JwtAuthenticationTokenFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    String[] permitRequests = jwtSecurityParamBean.getPermitRequests();
    List<String> fuzzyRequests = Arrays.stream(permitRequests).filter(s -> s.contains("**")).collect(Collectors.toList());
    boolean isFuzzyPass = fuzzyRequests.stream()
        .map(s -> s.replace("/**", "").replace("**/", "").replace("**", ""))
        .anyMatch(requestURI::contains);
    List<String> refinedRequests = Arrays.stream(permitRequests).filter(s -> !s.contains("**")).collect(Collectors.toList());
    boolean isRefinedPass = refinedRequests.stream().anyMatch(requestURI::equals);
    if (!isFuzzyPass && !isRefinedPass) {
      try {
  ...
  private void setResponse(HttpServletResponse response, HttpServletRequest httpServletRequest) {
    ...
    if (!coreProperties.isDebug()) {
      ...
      if (jwtSecurityParamBean.getAllowOrigins().stream().anyMatch(s -> s.equals("*"))) {
        allowOrigin.set("*");
      } else if (origin != null) {
    ...
    if (!allowOrigin.get().equals("null")) {
